    ...
    for (int h = 0; h < height; h++)
    {
        for (int w = 0; w < width; w++)
        {
            ...
            for (int i = 0; i <= 2; i++)
            {
                for (int j = 0; j <= 2; j++)
                {
                    sumgy.rgbtBlue += gx[i][j] * copyimage[h+i-1][w+j-1].rgbtBlue;
                    sumgy.rgbtGreen += gx[i][j] * copyimage[h+i-1][w+j-1].rgbtGreen;
                    sumgy.rgbtRed += gx[i][j] * copyimage[h+i-1][w+j-1].rgbtRed;
                }
            }
